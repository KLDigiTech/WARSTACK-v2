...
export async function initMessages() {
  ...
  document.querySelectorAll('.var-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const val = ta.value;
      ta.value = val.slice(0, pos) + btn.dataset.var + val.slice(pos);
      ta.focus();
  ...
  // Fréquence → afficher/masquer jour de semaine
  document.getElementById('campaign-frequency').addEventListener('change', () => {
    ...
    document.getElementById('weekday-group').style.display =
      freq === 'weekly' ? '' : 'none';
  });
...
async function loadCampaigns() {
  const data = await fetchSupabase(
    `recurring_messages?guild_id=eq.${GUILD_ID}&order=created_at.asc`
  ) || [];
...
function renderList() {
        ...
        <div class="campaign-card-header">
          <div>
            ...
            <div class="campaign-meta">
              ${ch ? `#${ch.name}` : '—'} · ${freqLabel(c.frequency)}
              · ${String(c.send_hour).padStart(2,'0')}:${String(c.send_minute).padStart(2,'0')}
            </div>
          </div>
  ...
  el.querySelectorAll('.toggle-campaign').forEach(chk => {
    chk.addEventListener('change', async () => {
      await fetchSupabase(`recurring_messages?id=eq.${chk.dataset.id}`, 'PATCH', {
        enabled: chk.checked
      });
...
async function saveCampaign() {
  ...
  const payload = { guild_id: GUILD_ID, name, channel_id: channel, frequency: freq, send_hour: hour, send_minute: minute, message };
...
function freqLabel(f) {
  return {
    ...
    every3h : 'Toutes les 3h',
    every6h : 'Toutes les 6h',
    every12h: 'Toutes les 12h',
    daily   : 'Tous les jours',
    every2d : 'Tous les 2 jours',
    every3d : 'Tous les 3 jours',
...
function nextSendTime(c) {
  ...
  const freqMs = {
    ...
    daily   : 24 * 60 * 60 * 1000,
    every2d : 2  * 24 * 60 * 60 * 1000,
    every3d : 3  * 24 * 60 * 60 * 1000,
    weekly  : 7  * 24 * 60 * 60 * 1000,
    every2w : 14 * 24 * 60 * 60 * 1000,
